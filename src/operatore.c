...
void processo_operatore(int id, int shm_id, int sem_id, int msg_id) {
    ...
    while (1) {
        ...
        // ⚠️ Controlla se la simulazione è ancora attiva PRIMA di usare semafori
        if (stats->simulazione_attiva == 0) {
            printf("[OPERATORE %d] Simulazione terminata durante preparazione\n", id);
            break;
        }
        ...
        Messaggio risposta;
        risposta.mtype = MSG_TYPE_RISPOSTA + msg.id_utente;
        risposta.id_utente = msg.id_utente;
        strcpy(risposta.piatto, msg.piatto);
    ...
    exit(0);  // ✅ Termina correttamente il processo figlio
}